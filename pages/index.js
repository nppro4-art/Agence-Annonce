  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--black)', overflowX: 'hidden' }}>
      <style>{`
        ...
        .action-card:hover { background: var(--s2) !important; }
        .cta-btn { transition: all .2s; }
        .cta-btn:hover { transform: translateY(-2px); filter: brightness(1.08); }
        .trust-pill { transition: all .15s; }
        .trust-pill:hover { border-color: rgba(201,168,76,.3) !important; }

        /* ── RESPONSIVE MOBILE UNIQUEMENT ── */
        @media (max-width: 768px) {
          .cursor-glow { display: none !important; }
          .counters-float { display: none !important; }
          ...
          .actions-grid { grid-template-columns: 1fr !important; background: var(--black) !important; gap: 12px !important; }
          .testimonials-grid { grid-template-columns: 1fr !important; background: var(--black) !important; gap: 12px !important; }
      ...
      <nav style={{
        ...
        <div className="nav-desktop" style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
          ...
          <Link href="/auth/register">
            <button className="btn-primary" style={{ fontSize: 12, padding: '10px 22px', letterSpacing: 2 }}>Commencer</button>
          </Link>
        </div>
      ...
      {/* HERO */}
      <section className="hero-section" style={{ position: 'relative', padding: '100px 32px 80px', maxWidth: 1100, margin: '0 auto', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 780, position: 'relative', zIndex: 1 }}>
          ...
          <div className="fade-up d5 cta-row" style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap', marginBottom: 56 }}>
            <Link href="/auth/register?plan=business">
              <button className="btn-gold cta-btn" style={{ fontSize: 13, padding: '16px 36px' }}>
                Commencer — 5,99 EUR/sem
              </button>
            </Link>
            <Link href="/pricing">
              <button className="btn-ghost cta-btn">
                Voir les tarifs
              </button>
            </Link>
          </div>

          <div className="fade-up d6 trust-row" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {['Annulable a tout moment', 'Paiement Stripe', 'Resultat en 15s', 'Sans engagement'].map(t => (
              <span key={t} className="trust-pill" style={{ fontSize: 11, color: 'var(--muted2)', background: 'var(--s1)', border: '1px solid var(--border)', borderRadius: 2, padding: '4px 10px', letterSpacing: .3 }}>
                + {t}
              </span>
            ))}
          </div>
        ...
        {/* Compteurs flottants DESKTOP */}
        <div className="counters-float fade-up d6" style={{ position: 'absolute', right: 40, top: 120, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[
      ...
      {/* 3 ACTIONS */}
      <section className="section-pad" style={{ padding: '80px 32px', maxWidth: 1100, margin: '0 auto' }}>
          ...
          {[
            {
              ...
              badge: 'Abonne',
              desc: "L'IA analyse votre article et redige un titre accrocheur, une description convincante et les points forts qui vendent.",
              href: '/auth/register'
            },
            {
              num: '02',
              title: 'Repondre a un acheteur',
              badge: 'Abonne',
              desc: "Collez le message recu. L'IA vous donne la reponse parfaite — ton professionnel, conseil de negociation, prete a copier.",
              href: '/auth/register'
            },
            {
              num: '03',
              ...
              badge: 'Gratuit',
              desc: "Obtenez une fourchette de prix realiste basee sur le marche francais actuel. 3 estimations gratuites par jour.",
              href: '/pricing'
            },
          ].map((a, i) => (
            <div key={i} className="action-card" onClick={() => router.push(a.href)}
              style={{ background: 'var(--ink)', padding: '40px 32px', position: 'relative', overflow: 'hidden' }}
            >
      ...
      {/* TEMOIGNAGES */}
      <section className="section-pad" style={{ background: 'var(--s1)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '80px 32px' }}>
            ...
            {testimonials.map((t, i) => (
              <div key={i} style={{ background: 'var(--ink)', padding: '32px 28px', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 32, height: 32, background: 'var(--s3)', border: '1px solid var(--gold-border)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-label)', fontSize: 14, color: 'var(--gold2)', flexShrink: 0 }}>
                    {t.name[0]}
                  </div>
                  <div>
      ...
      {/* CTA FINAL */}
      <section className="section-pad" style={{ padding: '100px 32px', maxWidth: 800, margin: '0 auto', textAlign: 'center', position: 'relative' }}>
        ...
        <Link href="/auth/register?plan=business">
          <button className="btn-gold cta-btn" style={{ fontSize: 14, padding: '18px 48px', letterSpacing: 2 }}>
