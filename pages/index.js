    ...
    <div style={{ minHeight:'100vh',background:'var(--black)',overflowX:'hidden' }}>
      <style>{`
        ...
        @media(max-width:768px){
          ...
          .ag{grid-template-columns:1fr 1fr!important;gap:1px!important}
      ...
      <section className="sec-pad" style={{ padding:'80px 32px',maxWidth:1100,margin:'0 auto' }}>
        <div style={{ marginBottom:48 }}>
          ...
          <h2 style={{ fontFamily:'var(--font-display)',fontSize:'clamp(28px,4vw,46px)',fontWeight:400,letterSpacing:-.5,lineHeight:1.1 }}>
            8 outils pour vendre.<br/><span style={{ fontStyle:'italic',color:'var(--muted3)',fontWeight:300 }}>Un seul abonnement.</span>
          </h2>
        </div>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:1,background:'var(--border)',marginBottom:1 }}>
          {[
            { num:'01',title:'Creer une annonce',badge:'Subscriber Only',desc:"Formulaire complet par categorie. L'IA redige titre, description, points forts et version courte pour SMS.",},
            { num:'02',title:'Repondre aux acheteurs',badge:'Subscriber Only',desc:"L'IA analyse le message et repond avec toutes les infos de votre annonce. Conseil de negociation inclus.",},
            { num:'03',title:'Estimer le prix',badge:'Gratuit',desc:"Fourchette basse/moyenne/haute basee sur le marche francais actuel. 3 estimations gratuites par jour.",},
            { num:'04',title:'Generateur de titres',badge:'Gratuit',desc:"5 titres optimises en un clic pour maximiser les vues sur LeBonCoin, Facebook Marketplace et Vinted.",},
            { num:'05',title:'Detecteur prix abusif',badge:'Gratuit',desc:"Comparez votre prix avec le marche. Alerte si vous etes trop cher - evitez de rater votre vente.",},
            { num:'06',title:'Mode vente flash',badge:'Subscriber Only',desc:"Annonce ultra-agressive avec prix attractif et urgence maximale. Concu pour vendre en moins de 48h.",},
            { num:'07',title:'Checklist publication',badge:'Gratuit',desc:"6 points a verifier avant de publier votre annonce. Ne publiez plus sans etre sur que tout est parfait.",},
            { num:'08',title:'Traduction annonce',badge:'Subscriber Only',desc:"Traduisez votre annonce en anglais, espagnol, allemand, italien ou neerlandais pour vendre en Europe.",},
          ].map((a,i) => (
            <div key={i} className="ac" onClick={() => router.push('/auth/register')} style={{ background:'var(--ink)',padding:'28px 24px',position:'relative',overflow:'hidden' }}>
              <div style={{ position:'absolute',top:16,right:20,fontFamily:'var(--font-label)',fontSize:48,color:'var(--border2)',lineHeight:1,userSelect:'none' }}>{a.num}</div>
              <div style={{ marginBottom:10 }}>
                <span className={a.badge==='Subscriber Only'?'badge badge-gold':'badge badge-green'}>
                  {a.badge==='Subscriber Only'?'♛ ':''}{a.badge}
                </span>
              </div>
              <h3 style={{ fontFamily:'var(--font-display)',fontSize:18,fontWeight:600,letterSpacing:-.3,marginBottom:8,lineHeight:1.2 }}>{a.title}</h3>
              <p style={{ fontSize:12,color:'var(--muted2)',lineHeight:1.7 }}>{a.desc}</p>
      ...
      <section className="sec-pad" style={{ background:'var(--s1)',borderTop:'1px solid var(--border)',borderBottom:'1px solid var(--border)',padding:'80px 32px' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:48 }}>
            <div className="label" style={{ marginBottom:12 }}>Pourquoi s&apos;abonner ?</div>
            <h2 style={{ fontFamily:'var(--font-display)',fontSize:'clamp(24px,4vw,42px)',fontWeight:400,letterSpacing:-.5 }}>
              Moins cher qu&apos;un cafe.<br/>
              <span style={{ fontStyle:'italic',color:'var(--muted3)',fontWeight:300 }}>Bien plus rentable.</span>
            </h2>
          </div>
            ...
            {[
              {
                plan:'Starter',
                price:'3,99',
                compare:'un croissant au cafe',
                emoji:'🥐',
                comparePrice:'~3,50 EUR',
                annonces:5,
                reponses:20,
                exemple:'Vendez votre velo, votre telephone ou votre veste. 5 annonces pro par semaine.',
                color:'var(--muted3)',
              },
              {
                plan:'Business',
                price:'5,99',
                compare:'un cafe et une viennoiserie',
                emoji:'☕',
                comparePrice:'~5,50 EUR',
                annonces:15,
                reponses:60,
                exemple:'Vendez regulierement. Demenagement, vide grenier, renouvellement de materiel.',
                color:'var(--gold2)',
                recommended:true,
              },
              {
                plan:'Expert',
                price:'9,99',
                compare:'deux places de cinema',
                emoji:'🎬',
                comparePrice:'~10 EUR',
                annonces:40,
                reponses:250,
                exemple:'Vendeur actif ou revendeur. 40 annonces et 250 reponses chaque semaine.',
                color:'var(--cream)',
              },
            ].map((p,i) => (
              <div key={i} style={{ background:p.recommended?'var(--ink)':'var(--s2)',padding:'32px 28px',position:'relative',borderTop:p.recommended?'2px solid var(--gold)':'2px solid transparent' }}>
                {p.recommended && (
                  <div style={{ position:'absolute',top:0,left:'50%',transform:'translateX(-50%) translateY(-50%)',background:'var(--gold)',color:'#030303',fontFamily:'var(--font-label)',fontSize:8,letterSpacing:2,padding:'3px 10px',whiteSpace:'nowrap' }}>LE PLUS POPULAIRE</div>
                )}
                <div style={{ fontFamily:'var(--font-label)',fontSize:16,letterSpacing:2,color:p.color,marginBottom:6 }}>{p.plan}</div>
                <div style={{ fontFamily:'var(--font-label)',fontSize:36,color:p.color,letterSpacing:-2,lineHeight:1,marginBottom:16 }}>
                  {p.price}<span style={{ fontSize:12,color:'var(--muted2)',letterSpacing:0 }}> EUR/sem</span>
                </div>
                <div style={{ background:'rgba(255,255,255,.03)',border:'1px solid var(--border)',borderRadius:3,padding:'12px 16px',marginBottom:16 }}>
                  <div style={{ fontSize:11,color:'var(--muted2)',marginBottom:4 }}>Soit le prix de</div>
                  <div style={{ fontSize:22,marginBottom:2 }}>{p.emoji}</div>
                  <div style={{ fontSize:13,color:'var(--cream)',fontWeight:500 }}>{p.compare}</div>
                  <div style={{ fontSize:11,color:'var(--muted)',marginTop:2 }}>{p.comparePrice} dans un cafe parisien</div>
                </div>
                <div style={{ fontSize:12,color:'var(--muted2)',lineHeight:1.7,marginBottom:16 }}>{p.exemple}</div>
                <div style={{ display:'flex',gap:12 }}>
                  <div style={{ flex:1,background:'var(--s3)',borderRadius:2,padding:'8px',textAlign:'center' }}>
                    <div style={{ fontFamily:'var(--font-label)',fontSize:20,color:p.color,letterSpacing:-1 }}>{p.annonces}</div>
                    <div style={{ fontSize:9,color:'var(--muted2)',textTransform:'uppercase',letterSpacing:.5 }}>annonces</div>
                  </div>
                  <div style={{ flex:1,background:'var(--s3)',borderRadius:2,padding:'8px',textAlign:'center' }}>
                    <div style={{ fontFamily:'var(--font-label)',fontSize:20,color:p.color,letterSpacing:-1 }}>{p.reponses}</div>
                    <div style={{ fontSize:9,color:'var(--muted2)',textTransform:'uppercase',letterSpacing:.5 }}>reponses</div>
                  </div>
                </div>
              </div>
            ))}
