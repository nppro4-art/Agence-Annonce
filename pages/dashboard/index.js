...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/api/chat/list').then(r=>r.json()).catch(()=>({bots:[]})),
    ]).then(([a,r,c,ch]) => {
      const allAnn = a.annonces||[]
      const estimations = allAnn.filter(x=>x.type==='estimation').length
      const analyses = allAnn.filter(x=>x.type==='analyser').length
      const normalAnn = allAnn.filter(x=>x.type!=='estimation'&&x.type!=='analyser').length
      setAnnonces(allAnn)
      setUsage({ annonces:normalAnn, reponses:r.reponses?.length||0 })
      setTotalStats({ annonces:normalAnn, reponses:r.reponses?.length||0, estimations, analyses, chatbots:(ch.bots||[]).length })
  ...
  return (
    <div style={{ minHeight:'100vh',background:'#080a0f',fontFamily:'var(--font-ui)' }}>
      <style>{`
        ...
        .sidebar{position:fixed;left:0;top:0;bottom:0;width:60px;background:#0d1525;border-right:1px solid rgba(201,168,76,.1);z-index:200;display:flex;flex-direction:column;transition:width .28s cubic-bezier(.16,1,.3,1);overflow:hidden;box-shadow:4px 0 20px rgba(0,0,0,.4)}
        .sidebar:hover{width:220px}
        .sidebar-logo{display:flex;align-items:center;gap:14px;padding:0 18px;height:58px;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,.05);white-space:nowrap}
        .sidebar-item{display:flex;align-items:center;gap:16px;padding:0 18px;height:44px;cursor:pointer;border:none;background:transparent;color:var(--muted);transition:all .2s;white-space:nowrap;border-left:2px solid transparent;flex-shrink:0;width:100%;text-align:left;-webkit-tap-highlight-color:transparent}
        ...
        .sidebar-label{font-family:'Bebas Neue',sans-serif;font-size:13px;letter-spacing:1.5px;opacity:0;transition:opacity .2s .05s;overflow:hidden;text-overflow:ellipsis}
        .sidebar:hover .sidebar-label{opacity:1}
        .db-layout{margin-left:60px;min-height:100vh;display:flex;flex-direction:column}
        .db-topbar{position:sticky;top:0;z-index:150;height:48px;background:rgba(8,10,15,.97);border-bottom:1px solid rgba(255,255,255,.05);backdrop-filter:blur(20px);display:flex;align-items:center;justify-content:space-between;padding:0 20px;gap:10px}
        .db-main{flex:1;max-width:920px;padding:28px 24px 100px}
        .db-inp{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:6px;color:var(--white);font-size:13px;padding:10px 14px;width:100%;outline:none;transition:all .2s;font-family:var(--font-ui)}
        ...
        .btn-gold-db{display:inline-flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(135deg,#a8843c,#c9a84c);border:none;border-radius:6px;color:#030303;cursor:pointer;font-family:'Bebas Neue',sans-serif;font-size:13px;letter-spacing:2px;padding:13px 22px;transition:all .2s}
        ...
        .btn-ghost-db{display:inline-flex;align-items:center;justify-content:center;background:transparent;border:1px solid rgba(255,255,255,.1);border-radius:6px;color:var(--muted2);cursor:pointer;font-family:var(--font-ui);font-size:12px;padding:9px 16px;transition:all .2s}
        ...
        @media(max-width:768px){
          ...
          .db-topbar{padding:0 10px!important;gap:6px!important}
      ...
      {guideOpen && (
        <>
          <div onClick={()=>setGuideOpen(false)} style={{ position:'fixed',inset:0,zIndex:9997,background:'rgba(0,0,0,.45)',backdropFilter:'blur(3px)' }} />
      ...
      <nav className="sidebar">
        <div className="sidebar-logo">
          <Link href="/" style={{ fontFamily:'Bebas Neue,sans-serif',fontSize:16,letterSpacing:4,color:'var(--white)',textDecoration:'none',flexShrink:0 }}>
        ...
        <div style={{ flex:1 }} />
        {/* Numéro client 6 chiffres */}
        <div className="sidebar-item" style={{ cursor:'default',borderLeft:'none',opacity:.5 }}>
          <span className="sidebar-icon" style={{ fontSize:10,fontFamily:'DM Mono,monospace',color:'var(--muted)',width:'auto' }}>#{user?.clientCode||'------'}</span>
          <span className="sidebar-label" style={{ fontFamily:'DM Mono,monospace',fontSize:10,color:'var(--muted)',letterSpacing:1 }}>#{user?.clientCode||'------'}</span>
        </div>
      ...
      <div className="db-layout">
        ...
        <div className="db-topbar">
          {/* Compteurs gauche */}
          <div className="cpt-group" style={{ display:'flex',alignItems:'center',gap:10 }}>
            {[
          ...
          {/* Heure Paris au centre */}
          <ParisClock />

          {/* Badge plan + upgrade droite */}
          <div style={{ display:'flex',alignItems:'center',gap:8 }}>
            {isSubscribed ? (
              <div style={{ display:'flex',alignItems:'center',gap:0,borderRadius:6,overflow:'hidden',border:'1px solid rgba(201,168,76,.2)' }}>
                <div onClick={()=>setShowSubModal(true)} style={{ display:'flex',alignItems:'center',gap:6,background:'rgba(201,168,76,.07)',padding:'5px 12px',cursor:'pointer' }}>
                ...
                {!isPremium && (
                  <button onClick={()=>subscribe(planKey==='starter'?'business':'expert')}
                    ...
                    title={planKey==='starter'?'Passer à Business':'Passer à Expert'}>↑</button>
                )}
              </div>
            ) : (
        ...
        {/* ── MODAL ABONNEMENT ───── */}
        {showSubModal && (
          <div style={{ position:'fixed',inset:0,background:'rgba(8,10,15,.9)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:20,backdropFilter:'blur(8px)' }}
            onClick={()=>setShowSubModal(false)}>
            <div style={{ background:'#0c0e15',border:'1px solid rgba(201,168,76,.2)',borderRadius:12,padding:'28px 24px',width:'100%',maxWidth:440,position:'relative' }}
              onClick={e=>e.stopPropagation()}>
              <button onClick={()=>setShowSubModal(false)} style={{ position:'absolute',top:14,right:16,background:'rgba(255,255,255,.06)',border:'none',borderRadius:'50%',color:'var(--muted2)',cursor:'pointer',width:28,height:28,display:'flex',alignItems:'center',justifyContent:'center',fontSize:16 }}>×</button>
              <div style={{ fontFamily:'Bebas Neue,sans-serif',fontSize:9,letterSpacing:3,color:'var(--gold3)',marginBottom:8 }}>MON ABONNEMENT</div>
              <h3 style={{ fontFamily:'Cormorant Garamond,serif',fontSize:26,fontWeight:400,marginBottom:18 }}>Plan {PLAN_NAMES[planKey]}</h3>
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16 }}>
                {[['Plan',PLAN_NAMES[planKey]],['Prix',PLAN_PRICES[planKey]+(planKey!=='free'&&planKey!=='premium'?' €/sem':'')],['Statut',cancelDone?'Annulation planifiée':'Actif'],['Annonces',limits.annonces===Infinity?'∞':limits.annonces+'/sem'],['Réponses',limits.reponses===Infinity?'∞':limits.reponses+'/sem'],['Renouvellement','7 jours']].map(([l,v])=>(
                  <div key={l} style={{ background:'rgba(255,255,255,.03)',padding:'10px 14px',borderRadius:6 }}>
              ...
              {cancelDone ? (
                <div style={{ background:'rgba(45,122,79,.1)',border:'1px solid rgba(45,122,79,.3)',borderRadius:6,padding:'12px',fontSize:13,color:'var(--success2)',textAlign:'center' }}>
                  Annulation confirmée. Accès maintenu jusqu&apos;à fin de période.
                </div>
              ) : (
                <button onClick={cancelSubscription} disabled={cancelLoading}
                  style={{ width:'100%',background:'none',border:'1px solid rgba(200,57,43,.3)',borderRadius:6,color:'var(--red2)',cursor:'pointer',fontSize:12,padding:'11px',opacity:cancelLoading?0.6:1 }}>
                  {cancelLoading?'Annulation...':'Arrêter le prélèvement automatique'}
                </button>
              )}
            </div>
          </div>
        ...
        <main className="db-main">
          {tab==='home' && <HomeTab user={user} isSubscribed={isSubscribed} planKey={planKey} isPremium={isPremium} usage={usage} limits={limits} credits={credits} subscribe={subscribe} setShowSubModal={setShowSubModal} setTab={setTab} />}
          ...
          {tab==='annonce' && <AnnonceTab isSubscribed={isSubscribed} planKey={planKey} credits={credits} subscribe={subscribe} onUsed={()=>{setUsage(u=>({...u,annonces:u.annonces+1}));setTotalStats(t=>({...t,annonces:t.annonces+1}))}} />}
          {tab==='reponse' && <ReponseTab isSubscribed={isSubscribed} subscribe={subscribe} onUsed={()=>{setUsage(u=>({...u,reponses:u.reponses+1}));setTotalStats(t=>({...t,reponses:t.reponses+1}))}} />}
          {tab==='estimation' && <EstimationTab annonces={annonces} onUsed={()=>setTotalStats(t=>({...t,estimations:t.estimations+1}))} />}
...
const GUIDES = {
  home: { titre:'Tableau de bord', sections:[
    ...
    ['Crédits annonces', "Remis à zéro chaque semaine. Starter = 10/sem, Business = 30/sem, Expert = illimité."],
    ['Crédits réponses', "Chaque réponse IA consomme 1 crédit. Rechargement chaque lundi."],
  ]},
  annonce: { titre:'Créer une annonce', sections:[
    ...
    ['Barre de complétion', "Plus vous remplissez de champs, meilleure sera l\'annonce. Visez 70% minimum."],
    ['Score de qualité', "Après génération, l\'IA note votre annonce sur 100. Un score > 80 est excellent."],
    ['Case à cocher ☑', "Chaque champ du formulaire a une case à cocher. Si vous la cochez, ce champ sera exclu de l\'annonce finale générée. Utile pour garder certaines infos confidentielles (ex: prix minimum, notes personnelles)."],
  ...
  reponse: { titre:'Répondre à un acheteur', sections:[
    ...
    ['Lier une annonce', "En liant une annonce, l\'IA connaît votre article et son prix."],
    ['Conseil de négociation', "En plus de la réponse, l\'IA vous donne un conseil privé sur la négociation."],
  ]},
  estimation: { titre:'Estimer le prix', sections:[
    ["Comment ça marche", "Décrivez votre article. L\'IA analyse les prix du marché pour une fourchette basse / conseillée / haute."],
  ...
  analyser: { titre:'Analyser une annonce', sections:[
    ["Comment ça marche", "Collez une annonce existante. L\'IA la note, identifie les 3 problèmes, puis la réécrit."],
    ['Avant / Après', "Score original vs score amélioré. En moyenne +35 points."],
    ['Plan requis', "Disponible à partir du plan Business."],
  ]},
  ventes: { titre:'Mes ventes', sections:[
    ['Ajouter une vente', "Cliquez Ajouter, renseignez le titre et le prix demandé."],
    ['Marquer vendu', "Entrez le prix final. L\'outil calcule si vous avez vendu au-dessus ou en dessous."],
    ['Statistiques', "En haut : objets en cours, vendus, total gagné."],
  ]},
  chatbots: { titre:'Chatbots vendeur', sections:[
    ['Principe', "Un assistant IA lié à votre annonce répond aux acheteurs 24h/24 via un lien unique."],
    ['Ajouter à LeBonCoin', "Copiez le texte \"Copier pour LeBonCoin\" et collez-le dans votre description."],
    ['Désactiver', "Désactivez le chatbot une fois l\'article vendu."],
    ['Limites', "Starter = non disponible · Business = 50 msg/j · Expert = 200 msg/j · Premium = 500 msg/j."],
  ]},
  outils: { titre:'9 outils IA', sections:[
    ['Titres', "5 titres optimisés pour LeBonCoin."],
    ['Prix abusif', "L\'IA compare le prix avec le marché."],
    ['Calendrier', "Meilleur jour et heure pour publier."],
    ['Arnaque', "Analyse un message suspect, verdict rouge / orange / vert."],
    ['Mode flash', "Annonce agressive pour vendre en 48h."],
    ['Mode lot', "Annonce groupée pour plusieurs objets."],
  ]},
  historique: { titre:'Historique', sections:[
    ['Annonces', "Toutes vos annonces générées. Cliquez pour voir et copier."],
    ['Réponses', "Toutes vos réponses générées."],
    ['Conservation', "Historique conservé indéfiniment."],
  ]},
  tarifs: { titre:'Tarifs et plans', sections:[
    ['Fonctionnement', "Plans hebdomadaires. Annulation possible à tout moment."],
    ['Starter vs Business', "Starter = outils de base. Business = tout + chatbot + arnaque + analyser. Expert = illimité."],
    ['Packs', "Crédits supplémentaires sans abonnement."],
    ['Paiement', "Stripe sécurisé. Annulable depuis Profil."],
  ]},
  profil: { titre:'Profil et paramètres', sections:[
    ['Modifier email', "Dans Sécurité : nouvel email + mot de passe. Confirmation requise."],
    ['Double auth', "Code à 6 chiffres par email à chaque connexion."],
    ['Numéro client', "Votre code à 6 chiffres unique est affiché en bas de la sidebar. Utilisez-le pour contacter le support."],
    ['Parrainage', "Un ami qui souscrit = 1 semaine Business gratuite."],
  ]},
}
...
function GuidePanel({ tab, onClose }) {
  ...
  return (
    <div style={{ position:'fixed',top:0,right:0,bottom:0,width:340,maxWidth:'90vw',background:'rgba(6,8,14,.99)',border:'1px solid rgba(201,168,76,.2)',borderRadius:'12px 0 0 12px',zIndex:9999,backdropFilter:'blur(20px)',display:'flex',flexDirection:'column',boxShadow:'-20px 0 60px rgba(0,0,0,.7)' }}>
      <style>{`@keyframes slideInGuide{from{transform:translateX(100%)}to{transform:translateX(0)}}`}</style>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 20px',borderBottom:'1px solid rgba(255,255,255,.06)',flexShrink:0 }}>
        ...
        <button onClick={onClose} style={{ background:'rgba(255,255,255,.06)',border:'none',borderRadius:'50%',width:30,height:30,cursor:'pointer',color:'var(--muted2)',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center' }}>✕</button>
      </div>
        ...
        {guide.sections.map(([titre, contenu], i) => (
          <div key={i} style={{ borderBottom:'1px solid rgba(255,255,255,.04)' }}>
            <button onClick={()=>setOpenIdx(openIdx===i?null:i)}
              ...
              <span style={{ fontFamily:'Bebas Neue,sans-serif',fontSize:11,letterSpacing:1,color:openIdx===i?'var(--gold2)':'var(--cream)' }}>{titre}</span>
            ...
            {openIdx===i && (
              <div style={{ padding:'0 20px 16px 34px',fontSize:12,color:'var(--muted2)',lineHeight:1.8 }}>
                {contenu}
              </div>
            )}
          </div>
      ...
      <div style={{ padding:'12px 20px',borderTop:'1px solid rgba(255,255,255,.04)',fontSize:10,color:'var(--muted)',textAlign:'center',flexShrink:0 }}>
...
// ─── HOME TAB ─────────────────────────────────────────────
function HomeTab({ user, isSubscribed, planKey, isPremium, usage, limits, credits, subscribe, setShowSubModal, setTab }) {
  return (
    <div className="db-fade">
      <LaunchBanner />
      <div style={{ marginBottom:32,paddingBottom:28,borderBottom:'1px solid rgba(201,168,76,.1)' }}>
        <div style={{ fontFamily:'Bebas Neue,sans-serif',fontSize:10,letterSpacing:3,color:'var(--muted2)',textTransform:'uppercase',marginBottom:10 }}>Tableau de bord</div>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:8 }}>
          ...
          <span style={{ fontSize:12,color:'var(--muted2)' }}>Plan <span style={{ color:isSubscribed?'var(--gold2)':'var(--muted2)',fontFamily:'DM Mono,monospace',fontSize:11 }}>{PLAN_NAMES[planKey]}</span>
            {isSubscribed&&planKey!=='premium'&&<span style={{ color:'var(--muted)',marginLeft:6 }}>· {PLAN_PRICES[planKey]} €/sem</span>}
          </span>
          {isSubscribed
            ? <button onClick={()=>setShowSubModal(true)} style={{ marginLeft:4,background:'none',border:'none',color:'var(--muted)',cursor:'pointer',fontSize:10,padding:0,textDecoration:'underline' }}>gérer</button>
            : <button onClick={()=>subscribe('business')} style={{ marginLeft:4,background:'none',border:'none',color:'var(--gold3)',cursor:'pointer',fontSize:10,padding:0,textDecoration:'underline' }}>s&apos;abonner</button>
          }
        </div>
      </div>
      <ScoreVendeur usage={usage} />
      <div style={{ marginBottom:8 }}>
        <div style={{ fontFamily:'Bebas Neue,sans-serif',fontSize:10,letterSpacing:3,color:'var(--muted2)',textTransform:'uppercase',marginBottom:14 }}>Actions rapides</div>
        <div className="db-actions" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:28 }}>
          {[
            {title:'Créer',sub:'une annonce',t:'annonce',icon:'✍',desc:'Annonce complète en 15 secondes'},
            ...
            <div key={a.t} onClick={()=>setTab(a.t)}
              style={{ position:'relative',overflow:'hidden',background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.08)',borderRadius:14,padding:'22px 18px',cursor:'pointer',transition:'all .25s' }}
      ...
      <div style={{ marginBottom:8 }}>
        <div style={{ fontFamily:'Bebas Neue,sans-serif',fontSize:10,letterSpacing:3,color:'var(--muted2)',textTransform:'uppercase',marginBottom:14 }}>Utilisation cette semaine</div>
        <div className="db-grid2" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:28 }}>
          {[
            {label:'Annonces',val:usage.annonces,limit:limits.annonces,color:'var(--gold)',icon:'✍',packRemaining:credits.annonces?.remaining||0},
            {label:'Réponses',val:usage.reponses,limit:limits.reponses,color:'var(--red)',icon:'◎',packRemaining:credits.reponses?.remaining||0},
          ].map(s=>{
            ...
            return (
              <div key={s.label} style={{ background:'var(--s1)',border:'1px solid var(--border)',borderRadius:10,padding:'20px 22px',position:'relative',overflow:'hidden' }}>
                ...
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14 }}>
                  <span style={{ fontSize:10,letterSpacing:1.5,color:'var(--muted)',textTransform:'uppercase',fontFamily:'Bebas Neue,sans-serif' }}>{s.label}</span>
                  <span style={{ fontSize:16 }}>{s.icon}</span>
                </div>
                <div style={{ display:'flex',alignItems:'baseline',gap:4,marginBottom:12 }}>
                  <span style={{ fontFamily:'DM Mono,monospace',fontSize:32,fontWeight:500,color:'var(--cream)',letterSpacing:-1,lineHeight:1 }}>{isSubscribed?s.val:s.packRemaining}</span>
                  {isSubscribed&&<span style={{ fontFamily:'DM Mono,monospace',fontSize:13,color:'var(--muted2)' }}>/{s.limit===Infinity?'∞':s.limit}</span>}
                </div>
                ...
                <div style={{ fontSize:10,color:'var(--muted)' }}>
                  {isSubscribed?(s.limit===Infinity?'Illimité':remaining+' restante'+(remaining!==1?'s':'')):(s.packRemaining===0?'Aucun crédit':s.packRemaining+' crédit(s)')}
                </div>
              </div>
            )
          })}
      ...
      {!isSubscribed && (
        <div style={{ background:'linear-gradient(135deg,rgba(201,168,76,.06),rgba(201,168,76,.02))',border:'1px solid rgba(201,168,76,.2)',borderRadius:10,padding:'24px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,flexWrap:'wrap' }}>
          <div>
            <div style={{ fontFamily:'Cormorant Garamond,serif',fontSize:20,fontWeight:500,marginBottom:4 }}>Débloquez toutes les fonctionnalités</div>
            <div style={{ fontSize:12,color:'var(--muted2)' }}>Starter 3,99 € · Business 5,99 € · Expert 12,99 €/semaine</div>
          </div>
          <button onClick={()=>setTab('tarifs')} style={{ background:'linear-gradient(135deg,var(--gold3),var(--gold2))',border:'none',borderRadius:6,color:'#030303',cursor:'pointer',fontFamily:'Bebas Neue,sans-serif',fontSize:12,letterSpacing:2,padding:'12px 24px',flexShrink:0 }}>
            VOIR LES TARIFS
          </button>
        </div>
      )}
...
function AnnonceTab({ isSubscribed, credits, subscribe, onUsed }) {
  ...
  const toggleExclude = (key) => setExcluded(prev => ({...prev, [key]:!prev[key]}))

  const generate = async () => {
    if (!categorie) { alert('Choisissez une categorie'); return }
    setLoading(true)
    try {
      const specs = 'Categorie: '+categorie+'\n'+Object.entries(form).filter(([k,v])=>v&&!excluded[k]).map(([k,v])=>k+': '+v).join('\n')
      ...
      {categorie&&(
        <>
          ...
          <div className="db-form-grid" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:8 }}>
            {fields.map(f=>(
              <div key={f.key} style={{ background:excluded[f.key]?'rgba(255,255,255,.01)':'var(--s1)',border:'1px solid',borderColor:excluded[f.key]?'rgba(255,255,255,.05)':'var(--border)',borderRadius:4,padding:'14px 16px',gridColumn:f.wide?'1/-1':'auto',opacity:excluded[f.key]?.5:1,transition:'all .2s' }}>
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6 }}>
                  <label style={{ ...S.lbl,marginBottom:0 }}>{f.label}</label>
                  <button onClick={()=>toggleExclude(f.key)} title={excluded[f.key]?'Inclure dans l\'annonce':'Exclure de l\'annonce'}
                    style={{ display:'flex',alignItems:'center',gap:4,background:'none',border:'1px solid',borderColor:excluded[f.key]?'rgba(200,57,43,.3)':'rgba(255,255,255,.08)',borderRadius:4,cursor:'pointer',padding:'2px 7px',fontSize:9,color:excluded[f.key]?'var(--red2)':'var(--muted)',transition:'all .2s',fontFamily:'Bebas Neue,sans-serif',letterSpacing:1 }}>
                    {excluded[f.key]?'✕ EXCLU':'☐ INCLURE'}
                  </button>
                </div>
                {f.type==='select'
                  ? <select style={{ ...S.inp,appearance:'none',cursor:'pointer' }} value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})} disabled={excluded[f.key]}>
                      <option value="">Sélectionner</option>
                      {f.opts.map(o=><option key={o}>{o}</option>)}
                    </select>
                  : f.wide
                    ? <textarea style={{ ...S.inp,resize:'vertical',minHeight:52,lineHeight:1.6 }} placeholder={f.ph} value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})} disabled={excluded[f.key]} />
                    : f.type==='urgence'
                      ? <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginTop:4 }}>
                          {[['normal','Normal'],['rapide','Rapide'],['optimise','Max prix']].map(([v,l])=>(
                            <div key={v} onClick={()=>!excluded[f.key]&&setForm({...form,urgence:v})}
                              style={{ background:form.urgence===v?'rgba(201,168,76,.08)':'var(--s2)',borderBottom:form.urgence===v?'2px solid var(--gold)':'2px solid transparent',padding:'7px',textAlign:'center',fontSize:11,color:form.urgence===v?'var(--gold2)':'var(--muted2)',cursor:excluded[f.key]?'not-allowed':'pointer' }}>
                              {l}
                            </div>
                          ))}
                        </div>
                      : <input style={S.inp} type={f.type==='number'?'text':'text'} inputMode={f.type==='number'?'numeric':undefined} placeholder={f.ph} value={form[f.key]||''} onChange={e=>setForm({...form,[f.key]:e.target.value})} disabled={excluded[f.key]} />
                }
              </div>
            ))}
          </div>
...
function EstimationTab({ annonces = [], onUsed }) {
  ...
  const estimate = async () => {
    ...
    if (!data.error) onUsed?.(); setResult(data); setLoading(false)
  }
  return (
    <div className="db-fade">
